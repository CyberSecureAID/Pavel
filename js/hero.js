...
(function () {
  ...
  /* ════════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════════
     El header debe ocultarse cuando el scroll supera el 50% de la
     altura de la sección "inicio" (#inicio), es decir, MIENTRAS AÚN
     ESTAMOS dentro del Hero — NUNCA al entrar a #servicios ni a
     ninguna otra sección. Si el trigger se calcula sobre el offsetTop
     de #servicios (o cualquier otra sección), el header queda
     superpuesto sobre el título de Servicios (bug visual recurrente).
     Este error ya se ha corregido manualmente varias veces porque
     ediciones posteriores del index/hero.js lo reintroducen sin
     querer. Bajo ninguna circunstancia cambies la referencia de
     "heroSec" ni el umbral de 0.5 (50%) usado más abajo en
     "enteredServicios". Si necesitas ajustar el punto de ocultado,
     modifica SOLO el multiplicador 0.5 — no cambies de sección base.
     ════════════════════════════════════════════════════════════════ */
  ...
  function onScroll() {
    ...
    /* ⚠️ Ver advertencia arriba — NO cambiar la base de este cálculo.
       Se oculta al pasar el 50% de la altura de #inicio (heroSec),
       NUNCA al llegar a #servicios u otra sección. */
    if (header && heroSec) {
      const enteredServicios = sy >= heroSec.offsetHeight * 0.5;
