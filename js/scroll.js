...
(function () {
  ...
  function findAnswer(rawText) {
    ...
    for (var k = 0; k < allScored.length && selected.length < MAX_INTENTS; k++) {
      ...
      selected.push({ text: answer, entryIndex: item.entryIndex, confidence: item.confidence, showContactCard: !!item.entry.showContactCard });
    }
    ...
    return { text: finalText, type: 'kb', confidence: bestConf, showContactCard: selected.some(function(s){ return s.showContactCard; }) };
  }
  ...
  function addContactCard() {
    ...
    var emailRow = contact.email
      ? '<a class="pv-contact-card pv-bubble-anim" href="mailto:' + contact.email + '" style="margin-top:6px;">' +
  ...
  function sendMessage(text) {
    ...
    if (domain === 'empresa') {
      ...
      if (kbResult && kbResult.text) {
        ...
        if (kbResult.showContactCard) setTimeout(addContactCard, estimateReplyDuration(kbResult.text) + 300);
        return;
      }
    ...
    if (result && result.text) {
      ...
      if (result.showContactCard) setTimeout(addContactCard, estimateReplyDuration(result.text) + 300);
      return;
    }
