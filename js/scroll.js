...
/* ================================================
   CHATBOT WIDGET v3.0 — Motor Conversacional IA
...
(function () {
  ...
  var PV_DATA_FALLBACK = {
    ...
    welcome: '¡Hola! 👋 Soy Leo, el asistente virtual de Maxcon. ¿En qué puedo ayudarle hoy?',
    modeIntros: {
      empresa: 'Volvemos a hablar sobre la empresa. ¿Qué desea saber?',
      general: 'Modo conversación general activado. Pregúnteme lo que quiera.'
    },
    contact: { name: 'Equipo Maxcon', phoneDisplay: '+57 300 123 4567', phoneHref: '+573001234567' }
  };
  ...
  var MSG_TYPE_PATTERNS = {
    ...
    orden:    [/^(dame|d[aá]me|env[ií]ame|mand|muestr|lista|dime|explica|describ|quiero)/,
               /^(necesito|busco|require|solicito)/],
  ...
  function classifyMessage(text) {
    ...
    Object.keys(MSG_TYPE_PATTERNS).forEach(function(type) {
  ...
  function findAnswer(rawText) {
    ...
    if (isAnaphora !== false && isAnaphora !== null) {
      ...
      if (prevEntry) {
        ...
        return { text: anaphoraAnswer, type: 'anaphora', confidence: 0.9 };
      }
    }
    ...
    for (var k = 0; k < allScored.length && selected.length < MAX_INTENTS; k++) {
      ...
      selected.push({ text: answer, entryIndex: item.entryIndex, confidence: item.confidence });
    }
    ...
    return { text: finalText, type: 'kb', confidence: bestConf };
  }

  /* ════════════════════════════════════════════════
     ENRUTADOR BIDIRECCIONAL ENTRE DOMINIOS
     ──────────────────────────────────────────────
     El selector de modo (La empresa / Tecnología) ya
     no es una pared rígida: solo define el COMPORTAMIENTO
     POR DEFECTO cuando la pregunta es ambigua (qué fallback
     mostrar, qué intro usar). El contenido real de cada
     mensaje se enruta según su propio dominio detectado:

       1. Si el mensaje matchea claramente la KB de la
          empresa (score >= DOMAIN_SCORE_THRESHOLD) →
          se responde con la KB, AUNQUE el modo activo
          sea "Tecnología".
       2. Si el mensaje no matchea la KB pero tiene forma
          de pregunta enciclopédica/general (qué es, quién
          fue, qué significa, etc.) o el modo activo es
          "Tecnología" → se consulta Wikipedia, AUNQUE el
          modo activo sea "La empresa".
       3. Si no hay match de KB y tampoco parece pregunta
          enciclopédica estando en modo empresa → se cae al
          comportamiento clásico de ese modo (aclaración o
          fallback con tarjeta de contacto), porque podría
          ser una consulta sobre la empresa mal formulada
          que no conviene desviar a Wikipedia.
  ════════════════════════════════════════════════ */
  ...
  /* Decide el dominio real del mensaje, sin importar el modo activo.
     Devuelve: 'empresa' | 'wikipedia' | 'ambiguo' */
  function detectDomain(rawText) {
    ...
    if (anaphoraTopic !== false && anaphoraTopic !== null) return 'empresa'; // referencia válida a un tema previo de la KB

    var companyScore  = bestCompanyScore(rawText);
    var isEncyclopedic = looksEncyclopedic(rawText);

    // Match fuerte de empresa (ej. "qué es BIM", "qué servicios ofrecen"):
    // gana sin importar la forma de la pregunta.
    if (companyScore >= DOMAIN_STRONG_THRESHOLD) return 'empresa';

    // Pregunta con forma claramente enciclopédica ("qué es X", "quién fue X"…)
    // y sin un match de empresa realmente fuerte: gana Wikipedia, incluso si
    // hubo un match débil casual con alguna key suelta de la KB.
    if (isEncyclopedic) return 'wikipedia';

    // Sin patrón enciclopédico, pero con un match débil-moderado de empresa
    // (ej. "cómo solicito una propuesta", donde solo matchea 1 de muchas keys):
    // se acepta como empresa.
    if (companyScore >= DOMAIN_WEAK_THRESHOLD) return 'empresa';

    // Sin match de empresa y sin forma enciclopédica explícita:
    // si está en modo Tecnología, igual preferimos Wikipedia (es su propósito);
    // si está en modo empresa, queda ambiguo y se resuelve con el flujo clásico
    // (que ahora también intenta Wikipedia como último recurso antes del fallback).
    if (currentMode === 'general') return 'wikipedia';
    return 'ambiguo';
  }
  ...
  var CASUAL_RESPONSES = {
    es: [
      'Hola, ¿en qué puedo ayudarle hoy? Estoy aquí para responder sus consultas sobre Maxcon.',
      '¡Buenas! Si tiene alguna pregunta sobre nuestros servicios, proyectos o tecnología BIM, con gusto le atiendo.',
      'Hola, es un gusto saludarle. ¿Tiene alguna consulta sobre ingeniería o proyectos de construcción?',
      '¡Buenos días! Si desea conocer nuestros servicios, experiencia o proceso de trabajo, adelante con su pregunta.'
    ],
    en: [
      'Hello! How can I help you today? I\'m here to answer your questions about Maxcon.',
      'Hi there! Feel free to ask me about our services, projects, or BIM technology.',
      'Good day! What can I help you with regarding engineering or construction projects?'
    ]
  };
  ...
  var CLARIFICATION_MSGS = {
    es: [
      ...
      'Para orientarle mejor, ¿puede decirme a qué área se refiere: servicios, proyectos, BIM, contacto o algo más?',
    ...
    en: [
      ...
      'To help you better, which area are you referring to: services, projects, BIM technology, contact, or something else?',
  ...
  function fetchWikipediaAnswer(rawQuery, onSuccess, onFail) {
    ...
    fetch(searchUrl, { signal: signal })
      ...
      .catch(function(err) { if (err && err.name === 'AbortError') return; onFail(); });
  }
  ...
  function sendMessage(text) {
    ...
    /* ─── Enrutador cross-dominio: decide el dominio REAL del
       mensaje, sin importar qué pestaña (modo) esté activa.
       Así, una pregunta de empresa hecha en modo "Tecnología"
       se responde con la KB, y una pregunta enciclopédica hecha
       en modo "La empresa" se responde vía Wikipedia. ─── */
    ...
    if (domain === 'empresa') {
      ...
      if (kbResult && kbResult.text) {
        ...
        botReply(kbResult.text);
        return;
      }
      // No debería pasar (detectDomain ya confirmó score fuerte),
      // pero por seguridad cae al flujo ambiguo de abajo.
    }

    /* ─── Dominio ambiguo (o modo empresa sin match fuerte):
       se conserva el comportamiento clásico — aclaración si hay
       indicios débiles, o fallback con tarjeta de contacto. ─── */
    ...
    if (!result || !result.text) {
      ...
      // Bajo confidence o no encontrado
      if (result && result.confidence > 0 && result.confidence < CONFIDENCE_THRESHOLD) {
        // Hay algo pero poca certeza → aclaración
      ...
      // Nada encontrado → como último recurso, intentar Wikipedia
      // antes de mostrar el fallback de contacto (cubre casos donde
      // el mensaje no calzó ningún patrón enciclopédico explícito
      // pero tampoco es realmente sobre la empresa).
